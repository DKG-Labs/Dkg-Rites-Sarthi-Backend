...
@RestController
@RequestMapping("/api/final-periodic-abrasion")
@RequiredArgsConstructor
@CrossOrigin("*")
public class RailFinalPeriodicAbrasionController {

    private final RailFinalPeriodicAbrasionService service;

    @PostMapping
    public ResponseEntity<APIResponse> save(@RequestBody RailFinalPeriodicAbrasionRequestDto requestDto) {
        RailFinalPeriodicAbrasionResponseDto response = service.save(requestDto);
        return ResponseEntity.ok(ResponseBuilder.getSuccessResponse(response));
    }

    @GetMapping("/{id}")
    public ResponseEntity<APIResponse> getById(@PathVariable Long id) {
        RailFinalPeriodicAbrasionResponseDto response = service.getById(id);
        return ResponseEntity.ok(ResponseBuilder.getSuccessResponse(response));
    }
    ...
    public ResponseEntity<APIResponse> getByCallNoAndLotNo(@PathVariable String callNo, @PathVariable String lotNo) {
        try {
            RailFinalPeriodicAbrasionResponseDto response = service.getByCallNoAndLotNo(callNo, lotNo);
            return ResponseEntity.ok(ResponseBuilder.getSuccessResponse(response));
        } catch (Exception e) {
    ...
    public ResponseEntity<APIResponse> getByCallNo(@PathVariable String callNo) {
        List<RailFinalPeriodicAbrasionResponseDto> response = service.getByCallNo(callNo);
        return ResponseEntity.ok(ResponseBuilder.getSuccessResponse(response));
    }
