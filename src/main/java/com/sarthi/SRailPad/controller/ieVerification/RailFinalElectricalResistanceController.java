...
@RestController
@RequestMapping("/api/final-electrical-resistance")
@RequiredArgsConstructor
@CrossOrigin("*")
public class RailFinalElectricalResistanceController {

    private final RailFinalElectricalResistanceService service;

    @PostMapping
    public ResponseEntity<APIResponse> save(@RequestBody RailFinalElectricalResistanceRequestDto requestDto) {
        RailFinalElectricalResistanceResponseDto response = service.save(requestDto);
        return ResponseEntity.ok(ResponseBuilder.getSuccessResponse(response));
    }

    @GetMapping("/{id}")
    public ResponseEntity<APIResponse> getById(@PathVariable Long id) {
        RailFinalElectricalResistanceResponseDto response = service.getById(id);
        return ResponseEntity.ok(ResponseBuilder.getSuccessResponse(response));
    }
    ...
    public ResponseEntity<APIResponse> getByCallNoAndLotNo(@PathVariable String callNo, @PathVariable String lotNo) {
        try {
            RailFinalElectricalResistanceResponseDto response = service.getByCallNoAndLotNo(callNo, lotNo);
            return ResponseEntity.ok(ResponseBuilder.getSuccessResponse(response));
        } catch (Exception e) {
    ...
    public ResponseEntity<APIResponse> getByCallNo(@PathVariable String callNo) {
        List<RailFinalElectricalResistanceResponseDto> response = service.getByCallNo(callNo);
        return ResponseEntity.ok(ResponseBuilder.getSuccessResponse(response));
    }
