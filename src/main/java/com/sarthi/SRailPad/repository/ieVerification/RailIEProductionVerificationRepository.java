...
public interface RailIEProductionVerificationRepository extends JpaRepository<RailIEProductionVerification, Long> {
    ...
    @Query(value = """
        SELECT COUNT(DISTINCT DATE(v.casting_date))
        FROM rail_ie_production_verification v
        WHERE v.casting_date >= :since
    ...
    @Query(value = """
        SELECT 
            ...
            'PROCESS' AS stage,
            SUM(COALESCE(v.total_pieces_produced, 0)) AS inspectedQty,
            SUM(COALESCE(v.total_accepted_pieces, 0)) AS acceptedQty,
            SUM(COALESCE(v.total_pieces_rejected, 0)) AS rejectedQty
        FROM rail_ie_production_verification v
        LEFT JOIN rail_vendor_plant rvp ON CONVERT(v.production_unit USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(rvp.plant_id USING utf8mb4) COLLATE utf8mb4_unicode_ci
        LEFT JOIN USER_MASTER um ON um.USERID = v.created_by
        LEFT JOIN ie_profile ip ON ip.employee_code = um.EMPLOYEE_CODE
        LEFT JOIN rail_production_declaration rpd ON v.request_id = rpd.id
        LEFT JOIN po_header ph ON CONVERT(ph.po_no USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(rpd.po_no USING utf8mb4) COLLATE utf8mb4_unicode_ci
    ...
    """, nativeQuery = true)
    java.util.List<Object[]> fetchProcessPerformance(
        @org.springframework.data.repository.query.Param("startDate") java.time.LocalDate startDate,
        @org.springframework.data.repository.query.Param("endDate") java.time.LocalDate endDate,
