...
public class reportsImpl implements reports {
        ...
        public DashboardSummaryDto getDashboardSummary() {
                ...
                dto.setRailPadPoIssued(poHeaderRepository.countPoByItemCatDescr("Rail Pads"));

                Long railPadQtyNos = getRailPadPoQuantityNos();

                dto.setRailPadPoQuantityNos(railPadQtyNos != null ? railPadQtyNos : 0L);

                Long railPadQtySet = getRailPadPoQuantitySet();

                dto.setRailPadPoQuantitySet(railPadQtySet != null ? railPadQtySet : 0L);



                List<Object[]> callCounts = railWorkflowTransactionRepository.getRailPadInspectionCallCounts();
        ...
        public List<StageRejectionDto> getParetoAnalysis(String startDate, String endDate, String product) {
                ...
                if (product != null && (product.equalsIgnoreCase("Rail Pad") || product.equalsIgnoreCase("Rail Pads"))) {
                        ...
                        try {
                                ...
                                List<Object[]> rows = railIEProductionVerificationRepository.findRailPadParetoAnalysisRejections(lStart, lEnd);
    ...
    public List<com.sarthi.dto.reports.RailPadShiftWiseProductionDto> getRailPadShiftWiseProductionReport(
            ...
            String vCode = (vendorCode == null || vendorCode.trim().isEmpty() || "All Manufacturers".equalsIgnoreCase(vendorCode)) ? null : vendorCode.trim();

            String pId = (plantId == null || plantId.trim().isEmpty() || "All Places".equalsIgnoreCase(plantId)) ? null : plantId.trim();



            List<Object[]> rows = railIEProductionVerificationRepository.getRailPadShiftWiseProductionReport(start, end, vCode, pId);
    ...
    public List<com.sarthi.dto.reports.RailPadQualityReportDto> getRailPadQualityReport(String startDate, String endDate) {

            LocalDate start = (startDate != null && !startDate.isEmpty()) ? LocalDate.parse(startDate) : LocalDate.now().minusYears(1);
            ...
            List<Object[]> verificationStatsList = railIEProductionVerificationRepository.findVerificationStatsGroupedByPo(start, end);
            ...
            List<Object[]> finalStatsList = railIEProductionVerificationRepository.findFinalInspectionStatsGroupedByPo(start, end);
            ...
            List<Object[]> rejectionsList = railIEProductionVerificationRepository.findProcessRejectionsGroupedByPo(start, end);
            ...
            for (PoHeader po : railPadPoHeaders) {
                    ...
                    if (verificationStats != null) {
                            ...
                            dto.setQtyInspected(verificationStats[1] != null ? ((Number) verificationStats[1]).longValue() : 0L);

                            dto.setQtyAccepted(verificationStats[2] != null ? ((Number) verificationStats[2]).longValue() : 0L);
                    ...
                    long rm = 0, compounding = 0, mixing = 0, curing = 0, cutting = 0, rheometer = 0, visualFinishing = 0;

                    long hardness = 0, gravity = 0, rubber = 0, ash = 0, rebound = 0, dimension = 0, weight = 0, surface = 0, compression = 0, visualTest = 0, other = 0;
                    ...
                    long totalRejections = rm + compounding + mixing + curing + cutting + rheometer + visualFinishing +

                                           hardness + gravity + rubber + ash + rebound + dimension + weight + surface + compression + visualTest + other;
