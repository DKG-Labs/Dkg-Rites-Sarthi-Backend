...
public class WorkflowServiceImpl implements WorkflowService {
        ...
        public void validateUser(Integer userId) {
                UserMaster userMaster = userMasterRepository.findById(userId)
                                .orElseThrow(() -> new InvalidInputException(new ErrorDetails(
                                                AppConstant.USER_NOT_FOUND, AppConstant.ERROR_TYPE_CODE_VALIDATION,
                                                AppConstant.ERROR_TYPE_VALIDATION, "User not found.")));
        }

        @Override
        @Transactional
        public WorkflowTransitionDto initiateWorkflow(String requestId, Integer createdBy, String workflowName,
                        String pincode) {
                ...
                // When workflow is initiated, update the inspection_call.created_by to the
                // actual user ID
                try {
                        ...
                        if (inspectionCall != null &&
                                ...
                                System.out.println("✅ Updated inspection_call.created_by to: " + createdBy + " for IC: "
                                                + requestId);
                        }
                } catch (Exception e) {
                ...
                if (workflowName.equalsIgnoreCase("IE INSPECTION")) {
                        ...
                        WorkflowTransition last = workflowTransitionRepository
                                        .findTopByRequestIdAndStatusOrderByWorkflowTransitionIdDesc(requestId,
                                                        "CALL_REGISTERED");
                        ...
                        if (ic.getTypeOfCall().equalsIgnoreCase("PROCESS")) {
                                ...
                                if (last.getAssignedToUser() == null ||
                                                !last.getAssignedToUser().equals(createdBy)) {// Only assigned IE can
                                                                                              // act
                        ...
                        if ("Final".equalsIgnoreCase(ic.getTypeOfCall())) {
                                ...
                                List<IePincodePoiMapping> ieMappings = iePincodePoiMappingRepository
                                ...
                                for (IePincodePoiMapping mapping : ieMappings) {
                                        ...
                                        UserMaster userOpt = userMasterRepository.findByEmployeeCode(employeeCode);
                ...
                if (workflowName.equalsIgnoreCase("INSPECTION CALL")) {
                         ...
                         * regionClusterRepository.findByClusterName(cluster.getClusterName())
                         * .orElseThrow(() -> new BusinessException(
                         * new ErrorDetails(
                         * AppConstant.ERROR_CODE_RESOURCE,
                         * AppConstant.ERROR_TYPE_CODE_RESOURCE,
                         ...
                         * ClusterRioUser rioUser =
                         * clusterRioUserRepository.findByClusterName(cluster.getClusterName())
                         * .orElseThrow(() -> new BusinessException(
                         * new ErrorDetails(
                         * AppConstant.ERROR_CODE_RESOURCE,
                         * AppConstant.ERROR_TYPE_CODE_RESOURCE,
                        ...
                        IEFieldsMapping mapping = ieFieldsMappingRepository
        ...
        private void validateProcessIeAction(Long processIeUserId, Long actionBy, String poicode) {
                ...
                List<Long> mappings = processIeUsersRepository
                                .findIeUsersByProcessIeAndPoi(Math.toIntExact(processIeUserId), poicode);
                ...
                if (mappings.isEmpty()) {
                        throw new BusinessException(
                                        new ErrorDetails(
                                                        ...
                                                        AppConstant.ERROR_TYPE_CODE_VALIDATION,
                                                        AppConstant.ERROR_TYPE_VALIDATION,
                                                        "No IE users mapped under Process IE user: "
                                                                        + processIeUserId));
                }
                        ...
                        throw new InvalidInputException(
                                        new ErrorDetails(
                                                        ...
                                                        "You are not authorized to perform this action. " +
                                                                        "User " + actionBy
                                                                        + " is not under Process IE user "
                                                                        + processIeUserId));
                }
        }

        /*
         * private void validateProcessIeAction(Integer processIeUserId, Integer
         * actionBy) {
         * 
         * 
        ...
        private static final List<String> PROCESS_SWIFTS = List.of("A", "B", "C", "G");
        ...
        public WorkflowTransitionDto performTransitionAction(TransitionActionReqDto req) {
                ...
                if (current.getWorkflowId() == 2) {
                        ...
                        // TransitionMaster nextTransition =
                        // transitionMasterRepository.findByTransitionName(req.getAction());
                        ...
                        if (req.getAction().equalsIgnoreCase("INITIATE_INSPECTION")) {
                                last = workflowTransitionRepository
                                                .findTopByRequestIdAndStatusOrderByWorkflowTransitionIdDesc(
                                                                req.getRequestId(), "IE_SCHEDULED");
                        } else {
                                last = workflowTransitionRepository
                        ...
                        if (last != null &&
                                        ("INSPECTION_COMPLETE_CONFIRM".equalsIgnoreCase(last.getStatus())
                                                        || "INSPECTION_COMPLETE_CONFIRM"
                                                                        .equalsIgnoreCase(last.getAction()))) {
                        ...
                        if (req.getAction().startsWith("IE_REQUEST_RESCHEDULE")
                                ...
                                if ("PROCESS".equalsIgnoreCase(inspectionType)) {
                                        ...
                                        Integer processIeUserId = getProcessIeUserFromPoi(ic.getPlaceOfInspection(),
                                                        last.getProcessIeUserId());
                                        ...
                                        PincodePoIMapping poi = pincodePoIMappingRepository
                                                        .findByPoiCode(ic.getPlaceOfInspection())
                                                        .orElseThrow(() -> new BusinessException(
                                                                        new ErrorDetails(
                                                                                        AppConstant.ERROR_CODE_RESOURCE,
                        ...
                        if ("ENTRY_INSPECTION_RESULTS".equalsIgnoreCase(req.getAction())) {
                                ...
                                if (ic.getTypeOfCall().equalsIgnoreCase("Process")) {
                                        ...
                                        int totalOfferedQty = processInspectionDetailsRepository
                                        ...
                                        int alreadyInspectedLotQty = processIeQtyRepository
                                                        .sumInspectedQtyByRequestIdAndLotNumber(
                                                                        req.getRequestId(),
                                                                        lotNo);

                                        int lotOfferedQty = processIeQtyRepository
                                        ...
                                        if ("PROCESS".equalsIgnoreCase(ic.getTypeOfCall())) {
                                                ...
                                                int inspectedQty = processIeQtyRepository
                                                                .sumInspectedQtyByRequestId(req.getRequestId());
                        ...
                        if (req.getActionBy().equals("INITIATE_INSPECTION")) {
                                ...
                                WorkflowTransition pendingQty = workflowTransitionRepository
                                                .findTopByRequestIdAndStatus(current.getRequestId(),
                                                                "QTY_EDIT_REQUESTED");

                                if (pendingQty != null) {
                                        System.out.println(
                                                        "Qty edit request expired automatically because inspection started.");
                                        // Optional: update status in DB
                                        pendingQty.setStatus("QTY_EDIT_EXPIRED");
                                        workflowTransitionRepository.save(pendingQty);
                                }
                        }
                        // If action is NOT VERIFY_PO_DETAILS then check whether last action was
                        // VERIFY_PO_DETAILS
                        if (req.getAction().equalsIgnoreCase("ENTER_SHIFT_DETAILS_AND_START_INSPECTION")) {
                                ...
                                if (last == null ||
                                                (!last.getAction().equalsIgnoreCase("VERIFY_PO_DETAILS") &&
                                                                !last.getAction().equalsIgnoreCase(
                                                                                "PAUSE_INSPECTION_RESUME_NEXT_DAY"))) {
                        ...
                        if ("PROCESS".equalsIgnoreCase(inspectionType)) {
                                ...
                                Integer processIeUserId = getProcessIeUserFromPoi(im.getPlaceOfInspection(),
                                                last.getProcessIeUserId());
                        ...
                        if (req.getAction().equalsIgnoreCase("ENTRY_INSPECTION_RESULTS")
                                ...
                                if (req.getMaterialName().equalsIgnoreCase("Raw material")
                                                || req.getMaterialName().equalsIgnoreCase("Final")) {
                                        String paymentType = "PAYABLE";
                                        boolean paymentDone = true;
                        ...
                        // CHANGE HERE
                        if (req.getAction().equalsIgnoreCase("REQUEST_CORRECTION_TO_CM")
                                        && roleNameById(nextTransition.getNextRoleId())
                                                        .equalsIgnoreCase("Control Manager")) {
                        ...
                        if ("INSPECTION_COMPLETE_CONFIRM".equalsIgnoreCase(next.getStatus())) {
                                ...
                                if (ic != null && "PROCESS".equalsIgnoreCase(ic.getTypeOfCall())) {
                                         ...
                                         * Long icId = ic.getId();
                                         * 
                                         * // Optional<ProcessInspectionDetails> list =
                                         * processInspectionDetailsRepository.findByIcIdCals(icId);
                                         * 
                                         * int totalOfferedQty =
                                ...
                                details.setCertificateNo(
                                                generateCertificateNo(rio, ic.getIcNumber(), user.getShortName()));
                ...
                switch (action) {
                        ...
                        case "PAYMENT_CONFIRMED_CANCEL": // when po verification stage try cancle but blocked due
                                                         // payment then use this trastion to cancle
                                return confirmCancelAfterPayment(current, req);
                        ...
                        case "CM_QTY_DECISION":
                                return cmQtyDecision(current, req); // cm decision after vendor request for qty change
                                                                    // of IC
        ...
        private WorkflowTransitionDto confirmCancelAfterPayment(WorkflowTransition current,
                        TransitionActionReqDto req) {

                TransitionMaster t = transitionMasterRepository.findByTransitionName("CONFIRM_CANCEL_AFTER_PAYMENT");
        ...
        private WorkflowTransitionDto cancelCallByIE(WorkflowTransition current, TransitionActionReqDto req) {
                ...
                if (!roleNameById(Integer.valueOf(current.getNextRole()))
                                .equalsIgnoreCase("IE")
                                && !roleNameById(Integer.valueOf(current.getNextRole()))
                ...
                TransitionMaster transition = transitionMasterRepository.findByTransitionNameAndWorkflowId(
                                "CANCEL_CALL",
        ...
        private WorkflowTransitionDto verifyCall(WorkflowTransition current, TransitionActionReqDto req) {
                ...
                TransitionMaster verifyTransition = transitionMasterRepository.findByTransitionName("VERIFY_CALL");
                ...
                // String inspectionType = req.getInspectionType(); // RAW_MATERIAL / FINAL /
                // PROCESS
                ...
                TransitionMaster callRegTransition = transitionMasterRepository.findByTransitionName("CALL_REGISTERED");
                ...
                if ("PROCESS".equalsIgnoreCase(inspectionType)) {
                        ...
                        if (req.getAssignUserId() != null) {
                                callReg.setAssignedToUser(req.getAssignUserId());
                        } else {
                                callReg.setAssignedToUser(
                                                assignIE(poi.getPinCode(), "ERC", stage, ic.getPlaceOfInspection()));
                        }
                }
                ...
                if ("Final".equalsIgnoreCase(inspectionType)) {
                        ...
                        // Fetch IE mappings using POI code
                        List<IePincodePoiMapping> ieMappings = iePincodePoiMappingRepository
                                        .findByPoiCode(insp.getPlaceOfInspection());
        ...
        private WorkflowTransitionDto returnToVendor(WorkflowTransition current, TransitionActionReqDto req) {
                ...
                WorkflowTransition next = createNextTransition(current, rejectTransition, "RETURNED",
                                req.getRemarks(), req);
                WorkflowTransition vendorCreated = workflowTransitionRepository
                                .findByStatusRequestIdAndCurrentRoleName("Created", req.getRequestId(), "Vendor");
        ...
        private WorkflowTransitionDto fixRouting(WorkflowTransition current, TransitionActionReqDto req) {
                ...
                WorkflowTransition last = workflowTransitionRepository
                                .findTopByRequestIdOrderByWorkflowTransitionIdDesc(req.getRequestId());
        ...
        private WorkflowTransitionDto vendorResubmit(WorkflowTransition current, TransitionActionReqDto req) {
                ...
                /*
                 * PincodeCluster cluster =
                 * pincodeClusterRepository.findByPincode(req.getPincode())
                 * .orElseThrow(() -> new BusinessException(
                 * new ErrorDetails(
                 * AppConstant.ERROR_CODE_RESOURCE,
                 * AppConstant.ERROR_TYPE_CODE_RESOURCE,
        ...
        private TransitionMaster resolveConditionalTransition(
                ...
                TransitionMaster previous = transitionMasterRepository.findById(current.getTransitionId())
                ...
                if (Objects.equals(previous.getCurrentRoleId(), previous.getNextRoleId())) {
                        ...
                        List<TransitionMaster> actionTransitions = transitionMasterRepository
                ...
                List<TransitionMaster> transitions = transitionMasterRepository
                                .findByWorkflowId(current.getWorkflowId())
                                .stream()
                ...
                for (TransitionMaster t : transitions) {
                        ...
                        if (t.getConditionId() != null) {
                                TransitionConditionMaster cond = transitionConditionMasterRepository
                                                .findById(t.getConditionId()).orElse(null);
        ...
        private Integer getSecondaryIeForCluster(String clusterName) {
                ...
                List<ClusterSecondaryIe> list = clusterSecondaryIeRepository
                                .findByClusterNameOrderByPriorityOrderAsc(clusterName);
        ...
        // Optional<InspectionCall> ic =
        // inspectionCallRepository.findByIcNumber(wt.getRequestId());
        ...
        // workflowTransitionRepository.findByRequestId(wt.getRequestId());

        // java.util.Set<String> startStatuses = new
        // java.util.HashSet<>(java.util.Arrays.asList("INITIATE_INSPECTION",
        // "IE_SCHEDULED", "INSPECTION_IN_PROGRESS"));
        // java.util.Set<String> endStatuses = new
        // java.util.HashSet<>(java.util.Arrays.asList("INSPECTION_COMPLETE_CONFIRM",
        // "INSPECTION_COMPLETE"));

        // java.util.Date start = transitions.stream()
        // .filter(t -> t.getStatus() != null &&
        // startStatuses.contains(t.getStatus().toUpperCase()))
        ...
        // .filter(t -> t.getStatus() != null &&
        // endStatuses.contains(t.getStatus().toUpperCase()))
        // .map(WorkflowTransition::getCreatedDate)
        ...
        private WorkflowTransitionDto mapWorkflowTransition(WorkflowTransition wt) {
                ...
                if (i != null && "Final".equalsIgnoreCase(i.getTypeOfCall())) {
                        ...
                        List<FinalIeMapping> mappings = finalIeMappingRepository
                                        .findByWorkflowTransitionId(wt.getWorkflowTransitionId());
        ...
        @Cacheable(value = "ieUsersByProcessPoi", key = "#processIeUserId + '_' + #poiCode")
        private List<Integer> getIeUsersByProcessIeAndPlaceOfInsp(
                        Integer processIeUserId,
                        String poiCode) {

                List<Long> ieUserIds = processIeUsersRepository
        ...
        private List<Integer> getIeUsersByProcessIeAndPoi(Integer processIeUserId, String poiCode) {
                ...
                List<ProcessIeUsers> ieMappings = processIeUsersRepository.findAllByProcessUserId(processIeUserId);
                ...
                for (ProcessIeUsers map : ieMappings) {
                        ...
                        boolean poiExists = iePoiMappingRepository.existsByIeUserIdAndPoiCode(ieUserId, poiCode);
                ...
                if (ieUserIds.isEmpty()) {
                        throw new BusinessException(
                                        new ErrorDetails(
                                                        ...
                                                        AppConstant.ERROR_TYPE_CODE_RESOURCE,
                                                        AppConstant.ERROR_TYPE_VALIDATION,
                                                        "No IE found for POI " + poiCode + " under Process IE "
                                                                        + processIeUserId));
                }
        ...
        /*
         * private TransitionMaster handlePaymentCheck(TransitionActionReqDto req,
         * List<TransitionMaster> transitions) {
         * 
         * //here we have to fetch data from payement master ic is payable or non
         * payable
         ...
         * .filter(t ->
         * t.getTransitionName().equalsIgnoreCase("CALL_CANCELLED_NON_PAYABLE"))
         * .findFirst().orElse(null);
         * 
         * TransitionMaster blocked = transitions.stream()
         * .filter(t ->
         * t.getTransitionName().equalsIgnoreCase("CALL_BLOCKED_PENDING_PAYMENT"))
         * .findFirst().orElse(null);
         * 
        ...
        private WorkflowTransitionDto handleFinancePaymentVerification(
                ...
                boolean paymentReceived = req.getPaymentReceivedStatus() != null &&
                                req.getPaymentReceivedStatus().equalsIgnoreCase("YES");

                if (!paymentReceived) {
                        // Payment not received → PARKED
                        TransitionMaster parked = transitionMasterRepository
                                        .findByTransitionName("PARKED_PAYMENT_NOT_RECEIVED");
                ...
                TransitionMaster callReg = transitionMasterRepository.findByTransitionName("CALL_REGISTERED");
        ...
        /*
         * @Override
         * public List<WorkflowTransitionDto> allPendingWorkflowTransition(String
         * roleName) {
         * 
        ...
        public List<WorkflowTransitionDto> allPendingWorkflowTransition(String roleName) {
                ...
                List<WorkflowTransition> pending = "IE".equalsIgnoreCase(roleName)
                ...
                Map<String, InspectionDataDto> inspectionMap = inspectionCallRepository.findLiteByIcNumberIn(requestIds)
                                .stream()
                                .collect(Collectors.toMap(
                                                InspectionDataDto::icNumber,
                                                Function.identity()));

                // Map<String, InspectionCall> inspectionMap =
                // inspectionCallRepository.findByIcNumberIn(requestIds) .stream()
                ...
                Map<String, PoHeader> poHeaderMap = poHeaderRepository.findByPoNoIn(poNos).stream()
                                .collect(Collectors.toMap(PoHeader::getPoNo, Function.identity(), (a, b) -> a));

                Map<String, Map<String, PoItem>> poItemMap = poItemRepository.findByPoHeader_PoNoIn(poNos).stream()
                                .collect(Collectors.groupingBy(item -> item.getPoHeader().getPoNo(),
                                                Collectors.toMap(PoItem::getItemSrNo, Function.identity(),
                                                                (a, b) -> a)));
                ...
                return pending.stream()
                                .map(wt -> mapWorkflow(wt, inspectionMap, finalIeMap, poHeaderMap, poItemMap))
                                .collect(Collectors.toList());
        }
        ...
        private WorkflowTransitionDto mapWorkflow(
                        ...
                        Map<String, PoHeader> poHeaderMap,
                        Map<String, Map<String, PoItem>> poItemMap) {
                ...
                if (i != null) {
                        dto.setPoNo(i.poNo());
                        dto.setVendorName(i.vendorName());
                        dto.setProductType(i.typeOfCall());
                        dto.setDesiredInspectionDate(
                                        String.valueOf(i.desiredInspectionDate()));
                        dto.setPoSerialNo(i.poSerialNo());
                        dto.setPlaceOfInspection(i.placeOfInspection());

                        PoHeader ph = poHeaderMap.get(i.poNo());
                        if (ph != null) {
                                dto.setRlyShortName(ph.getRlyShortName());
                        }

                        Map<String, PoItem> itemsForPo = poItemMap.get(i.poNo());
                        if (itemsForPo != null && i.poSerialNo() != null) {
                                // Try exact match first
                                PoItem pi = itemsForPo.get(i.poSerialNo());

                                // fallback if serial is '1' but in DB it is '01'
                                if (pi == null && i.poSerialNo().length() == 1) {
                                        pi = itemsForPo.get("0" + i.poSerialNo());
                                }

                                if (pi != null) {
                                        if (pi.getDeliveryDate() != null) {
                                                dto.setDpDate(pi.getDeliveryDate().toLocalDate().toString());
                                        }
                                        if (pi.getExtendedDeliveryDate() != null) {
                                                dto.setExtDpDate(pi.getExtendedDeliveryDate().toLocalDate().toString());
                                        }
                                }
                        }
                }
        ...
        public List<WorkflowTransitionDto> allPendingQtyEditTransitions(String roleName) {
                ...
                List<WorkflowTransition> pendingQty = workflowTransitionRepository.findPendingQtyEditByRole(roleName);
        ...
        private Integer getProcessIeUserFromPoi(String poiCode, Integer processIe) {
                ...
                IePoiMapping latestPoiIe = iePoiMappingRepository
                ...
                if (processIe != 0) {
                        ...
                        latestProcessIe = processIeUsersRepository
        ...
        public List<WorkflowTransitionDto> workflowTransitionHistory(String requestId) {
                ...
                if (Objects.nonNull(workflowTransitionList) && !workflowTransitionList.isEmpty()) {
                        workflowTransitionDtoList = workflowTransitionList.stream().sorted(
                                        Comparator.comparing(WorkflowTransition::getWorkflowSequence).reversed())
                                        .map(e -> {
                ...
                if (initiated != null) {
                        throw new BusinessException(
                                        new ErrorDetails(AppConstant.INVALID_WORKFLOW_TRANSITION,
                                                        AppConstant.ERROR_TYPE_CODE_VALIDATION,
                                                        AppConstant.ERROR_TYPE_VALIDATION,
                                                        "Inspection initiated. CM cannot approve or reject now."));
                }

                WorkflowTransition next = createNextTransition(
                                current,
                                nextT,
                                req.getCmDecision().equalsIgnoreCase("APPROVED") ? "QTY_APPROVED" : "QTY_REJECTED",
                                req.getRemarks(),
                                req);
        ...
        public List<WorkflowTransitionDto> allBlockedWorkflowTransitions() {
                ...
                List<WorkflowTransition> blocked = workflowTransitionRepository.findBlockedTransitions();
        ...
        public WorkflowDto workflowByWorkflowName(String workflowName) {
                ...
                if (Objects.nonNull(workflowName)) {
                        ...
                        if (Objects.nonNull(workflowMaster)) {
                                ...
                                throw new InvalidInputException(new ErrorDetails(AppConstant.WORKFLOW_NOT_FOUND,
                                                AppConstant.ERROR_TYPE_CODE_VALIDATION,
                                                AppConstant.ERROR_TYPE_VALIDATION, "Workflow not found."));
                        }
                } else {
                        throw new InvalidInputException(new ErrorDetails(AppConstant.USER_INVALID_INPUT,
                                        AppConstant.ERROR_TYPE_CODE_VALIDATION,
        ...
        private String workflowNameById(Integer workflowId) {
                if (Objects.nonNull(workflowId)) {
                        return workflowMasterRepository.findById(workflowId).orElse(new WorkflowMaster())
                                        .getWorkflowName();
                } else {
                        return null;
                }
        ...
        private void assignRescheduleUser(WorkflowTransition next, WorkflowTransition current,
                        TransitionActionReqDto req) {
        ...
        private Integer getSbuHeadUser(String pincode) {
                ...
                // Get cluster details
                RegionCluster regionCluster = regionClusterRepository.findByClusterName(cluster.get().getClusterName())
                                .orElseThrow(() -> new BusinessException(
                                                new ErrorDetails(AppConstant.ERROR_CODE_RESOURCE,
                                                                AppConstant.ERROR_TYPE_CODE_VALIDATION,
                                                                AppConstant.ERROR_TYPE_VALIDATION,
                                                                "Cluster not found: "
                                                                                + cluster.get().getClusterName())));

                String regionName = regionCluster.getRegionName();

                // Fetch SBU Head for that region
                RegionSbuHead sbu = regionSbuHeadRepository.findByRegionName(regionName)
                                .orElseThrow(() -> new BusinessException(
                                                new ErrorDetails(AppConstant.ERROR_CODE_RESOURCE,
                                                                AppConstant.ERROR_TYPE_CODE_VALIDATION,
                                                                AppConstant.ERROR_TYPE_VALIDATION,
