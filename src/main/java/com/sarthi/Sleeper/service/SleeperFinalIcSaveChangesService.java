...
@Slf4j
public class SleeperFinalIcSaveChangesService {

    private final SleeperFinalIcSaveChangesRepository sleeperFinalIcSaveChangesRepository;

    @Transactional(readOnly = true)
    public SleeperFinalIcSaveChangesDTO getByIcNumber(String icNumber) {
        if (icNumber == null || icNumber.trim().isEmpty()) {
            return null;
        }
        return sleeperFinalIcSaveChangesRepository.findByIcNumber(icNumber.trim())
                .map(this::mapToDTO)
                .orElse(null);
    }

    @Transactional
    public SleeperFinalIcSaveChangesDTO saveOrUpdate(SleeperFinalIcSaveChangesDTO dto) {
        ...
        SleeperFinalIcSaveChanges entity = sleeperFinalIcSaveChangesRepository.findByIcNumber(icNo)
                .orElse(new SleeperFinalIcSaveChanges());
        ...
        SleeperFinalIcSaveChanges saved = sleeperFinalIcSaveChangesRepository.save(entity);
        log.info("Saved SleeperFinalIcSaveChanges draft successfully for IC: {}", icNo);
        return mapToDTO(saved);
    }

    private SleeperFinalIcSaveChangesDTO mapToDTO(SleeperFinalIcSaveChanges entity) {
        return SleeperFinalIcSaveChangesDTO.builder()
