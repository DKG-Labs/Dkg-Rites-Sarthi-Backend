...
@RestController
@RequestMapping("/api/admixture-test")
public class AdmixtureTestController {

    @Autowired
    private AdmixtureTestService service;

    @PostMapping
    public ResponseEntity<Object> create(@RequestBody AdmixtureTestRequestDto dto) {
        return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(service.create(dto)), HttpStatus.OK);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Object> update(@PathVariable Long id, @RequestBody AdmixtureTestRequestDto dto) {
